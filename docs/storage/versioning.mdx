...
Versioning is **always enabled** on every Fil One bucket. It cannot be suspended or disabled. Every time you upload an object with the same key, Fil One creates a new version rather than overwriting the previous one.
...
Because versioning is always on and cannot be suspended, this behavior applies to all objects in all buckets.
