...
:::info `PutObjectRetention` is region-dependent
Setting or changing retention on an *individual* object version via `PutObjectRetention` is offered as a grantable access-key permission. It is **confirmed working in `us-east-1`** — including extending a retention period and, using the standard S3 bypass-governance-retention mechanism, shortening one (this is a protocol-level capability of the gateway, not a permission Fil One's own access keys can grant — see [Key concepts](#key-concepts)). It has **not yet been confirmed in `eu-west-1`** — until then, rely on the bucket-level default set at creation for guaranteed retention there.
:::
...
- To set retention on an individual version you would grant the `PutObjectRetention` granular permission — see the [region-dependent note above](#setting-per-object-retention): confirmed in `us-east-1`, not yet confirmed in `eu-west-1`.
...
| Legal preservation | Variable | Retention-based preservation of evidence, or S3 *Legal Hold* (confirmed in `us-east-1`, not yet confirmed in `eu-west-1`) |
...
- Retention enforcement at the storage gateway is **confirmed in `us-east-1`** and **not yet independently verified in `eu-west-1`** (see the note under [Setting per-object retention](#setting-per-object-retention)).
