...
:::warning `PutObjectRetention` is not yet verified
Setting or changing retention on an *individual* object version via `PutObjectRetention` is offered as a grantable access-key permission, but the Fil One console never calls it and gateway support has **not been tested**. The examples below are the standard S3 form and are provided for reference — do not build a workflow that depends on per-object retention calls until this is confirmed. If you need guaranteed retention today, rely on the bucket-level default set at creation.
:::
...
In standard S3, retention can be extended but never shortened: you can move `RetainUntilDate` further into the future but not closer, and shortening a Compliance retention period is rejected (with `403 AccessDenied`). Fil One's gateway is expected to follow this, but as noted above the behaviour has not been independently verified.
...
Bucket deletion is currently restricted regardless of retention state. The dashboard delete button is disabled, the console's delete endpoint is not yet implemented, and over the S3 API `DeleteBucket` is only available in `us-east-1` (not `eu-west-1`). Active Compliance retention is one more reason a bucket cannot be removed, but it is not the only one — so do not assume a bucket becomes freely deletable simply because its retention periods have expired. See [Buckets](/storage/buckets).
...
- To set retention on an individual version you would grant the `PutObjectRetention` granular permission — but see the [unverified note above](#setting-per-object-retention); the permission being grantable does not confirm the gateway honours the call.
...
| Legal preservation | Variable | Retention-based preservation of evidence (note: S3 *Legal Hold* is not yet available — use a retention period) |
...
- Retention enforcement at the storage gateway has **not been independently verified** (see the note under [Setting per-object retention](#setting-per-object-retention)).
