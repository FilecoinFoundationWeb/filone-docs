...
|------|---------------|-------------|
| **Governance** (default) | Users with bypass permission | Internal policy retention, development, audit-friendly workflows |
...
`GetBucketVersioning` and `GetBucketObjectLockConfiguration` are selectable permissions that you grant when creating a key — they are not included automatically, so add them if the key needs to read a bucket's versioning or Object Lock settings. Beyond that, Object Lock behavior depends on the key's permissions and the bucket's retention mode:
...
Keep in mind that retention outlives the trial: a lock you set during the trial still applies after the trial ends, and retention cannot be shortened once set.
