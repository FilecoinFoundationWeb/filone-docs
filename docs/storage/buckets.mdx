...
Buckets can only be deleted from the Fil One dashboard (see the note under [Creating a bucket](#creating-a-bucket)).

Before deleting, you must remove all objects from the bucket. You can do this via the dashboard or the S3 API:

<Tabs>
  <TabItem value="aws-cli" label="AWS CLI">
    ```bash
    # Delete all objects first
    aws s3 rm s3://my-bucket --recursive --endpoint-url https://eu-west-1.s3.fil.one
    ```

    Then delete the empty bucket from the dashboard.
  </TabItem>
