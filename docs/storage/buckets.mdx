...
7. Click **Create bucket** (the button reads **Create bucket and API key** if you expanded the key section). The dashboard then opens the new bucket's page.
...
<Tabs>
  <TabItem value="aws-cli" label="AWS CLI">
    ```bash
    aws s3 ls --endpoint-url https://s3.eu-west-1.filonecontent.com
    ```
  </TabItem>
...
A bucket must be empty before it can be deleted, with no objects or object versions left in it.

<Tabs>
  <TabItem value="app" label="App">
    Open **Buckets**, choose **Delete bucket** from the bucket's actions menu, and confirm. If the bucket still holds objects, the dashboard tells you it is not empty. Open the bucket and use **Empty bucket** to remove everything first. See [Buckets in the console](/app/buckets#deleting-a-bucket).

    Dashboard deletion works in every region, including `eu-west-1`. Only the Owner and Admin roles can delete buckets.
  </TabItem>

  <TabItem value="aws-cli" label="AWS CLI">
    ```bash
    aws s3 rb s3://my-bucket --endpoint-url https://s3.us-east-1.filonecontent.com
    ```

    `DeleteBucket` over the S3 API works in `us-east-1`, using a key that has the `DeleteBucket` permission. It is not available in `eu-west-1`, so use the dashboard there.
  </TabItem>
</Tabs>
