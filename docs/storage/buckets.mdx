...
:::note
Bucket deletion is not yet available. The delete button in the dashboard is visible but disabled while the underlying storage layer completes support for this operation. The S3 `DeleteBucket` API also returns `AccessDenied`. This will be enabled in a future release.
:::
