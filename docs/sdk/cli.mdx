...
The [AWS CLI](https://aws.amazon.com/cli/) works with Fil One out of the box. Install it, configure your credentials, and use `--endpoint-url https://s3.<region>.filonecontent.com` on every command — or set it as a profile default so you never have to type it.

:::note The endpoint depends on your bucket's region
Examples here use `eu-west-1`, the default region. For a bucket in `us-east-1`, use `https://s3.us-east-1.filonecontent.com` and region `us-east-1`. The endpoint, the region, and your access key must all refer to the same region — set up one profile per region if you use both.
:::
...
In `eu-west-1` the operation is not available over the S3 API. Delete the bucket from the [Fil One dashboard](https://app.fil.one) instead, via the bucket's actions menu in **Buckets** (Owner and Admin roles only). See [Buckets](/storage/buckets#deleting-a-bucket).
