...
| `PutObjectRetention` | ✅ Supported | Sets or extends retention on a version. Cannot be shortened. |
| `GetObjectLegalHold` | ❌ Not supported | Planned for a future release. |
| `PutObjectLegalHold` | ❌ Not supported | Planned for a future release. |
...
| Mode | Status |
|------|--------|
| Governance | ✅ Supported — the default when you enable Object Lock. Retention is enforced, but users holding the bypass permission can override it. |
...
| Client-side encryption | ✅ Optional | Encrypt data before upload using AWS SDK encryption utilities. Server-side encryption is still applied on top. |
