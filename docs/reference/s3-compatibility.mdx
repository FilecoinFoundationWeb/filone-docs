...
| Region | Endpoint |
|--------|----------|
| Europe (France) `eu-west-1` (default) | `https://s3.eu-west-1.filonecontent.com` |
| US East (Michigan) `us-east-1` | `https://s3.us-east-1.filonecontent.com` |
...
| Parameter | Value |
|-----------|-------|
| Endpoint | `https://s3.<region>.filonecontent.com` |
...
To limit what a key can reach, scope it to specific buckets when you create it. See [Bucket scope](/security/api-keys#bucket-scope).
...
| IAM users, roles, and bucket policies on the S3 API | Not yet supported | The S3 API has no IAM users, roles, or bucket policies. Access is controlled by each access key's permissions and bucket scope. Team members and roles exist in the console, in beta: see [Organization](/app/organization#roles-and-permissions). |
...
| IAM and policies | Full IAM with users, roles, and bucket policies. | No IAM users, roles, or bucket policies on the S3 API. Access is controlled by each access key's permissions and bucket scope. The console has organizations with member roles, in beta: see [Organization](/app/organization#roles-and-permissions). |
