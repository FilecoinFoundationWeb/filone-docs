...
:::note Object Lock in `eu-west-1` is not yet independently verified
The per-object Object Lock operations are confirmed against `us-east-1` only. A compatibility run against `eu-west-1` could not exercise them, because the run was unable to create an Object-Lock-enabled bucket in that region to test against. This is a **gap in test coverage, not a known failure**. If you are relying on Object Lock for a compliance requirement in `eu-west-1`, validate the behavior you need on your own bucket first.
:::
