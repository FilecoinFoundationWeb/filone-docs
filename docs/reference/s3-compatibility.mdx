...
| `PutObjectRetention` | ⚠️ Region-dependent | Confirmed working in `us-east-1` — set, extend, or (with the bypass permission) shorten a version's retention. Not yet confirmed in `eu-west-1`. |
...
| `PutObjectRetention` | ⚠️ Region-dependent | Confirmed working in `us-east-1`, including extending, shortening (correctly rejected without the bypass permission), and Governance-mode bypass. Not yet confirmed in `eu-west-1`. |
| `GetObjectLegalHold` | ⚠️ Region-dependent | Confirmed working in `us-east-1` — requires the matching access-key permission. Not yet confirmed in `eu-west-1`. |
| `PutObjectLegalHold` | ⚠️ Region-dependent | Confirmed working in `us-east-1`, including blocking delete while a hold is active — requires the matching access-key permission. Not yet confirmed in `eu-west-1`. |
