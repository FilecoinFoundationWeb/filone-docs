...
| `CopyObject` | ⚠️ Region-dependent | Confirmed working in `us-east-1`, including copying a specific `VersionId` onto the current key. Not yet confirmed in `eu-west-1` — download and re-upload as a fallback there. |
| `GetObjectRetention` | ✅ Supported | Returns the Object Lock retention for a specific version. |
| `PutObjectRetention` | ⚠️ Region-dependent | Confirmed working in `us-east-1` — set, extend, or (via S3's bypass-governance-retention mechanism) shorten a version's retention. Not yet confirmed in `eu-west-1`. |
| `GetObjectLegalHold` | ⚠️ Region-dependent | Confirmed working in `us-east-1` — requires the matching access-key permission. Not yet confirmed in `eu-west-1`. |
| `PutObjectLegalHold` | ⚠️ Region-dependent | Confirmed working in `us-east-1`, including blocking delete while a hold is active — requires the matching access-key permission. Not yet confirmed in `eu-west-1`. |
...
| `CopyObject` with `x-amz-copy-source-version-id` | ⚠️ Region-dependent | Confirmed working in `us-east-1` — copy a specific version onto the current key to promote it. Not yet confirmed in `eu-west-1`. |

The Fil One **dashboard** has no restore, roll-back, or "make current" action — you can list, download, and delete any prior version there, but not promote one to current. In `us-east-1`, you can promote an older version via the API with `CopyObject` (see above); in `eu-west-1` this has not yet been confirmed, so download the version you want and upload it again as a new version instead.
...
| `PutObjectRetention` | ⚠️ Region-dependent | Confirmed working in `us-east-1`, including extending, shortening (correctly rejected without it), and Governance-mode bypass via S3's bypass-governance-retention mechanism. Not yet confirmed in `eu-west-1`. |
| `GetObjectLegalHold` | ⚠️ Region-dependent | Confirmed working in `us-east-1` — requires the matching access-key permission. Not yet confirmed in `eu-west-1`. |
| `PutObjectLegalHold` | ⚠️ Region-dependent | Confirmed working in `us-east-1`, including blocking delete while a hold is active — requires the matching access-key permission. Not yet confirmed in `eu-west-1`. |
...
| SSE-KMS (`aws:kms`) | ❌ Not supported | Uploads specifying `ServerSideEncryption: aws:kms` are rejected in `us-east-1` with `InvalidRequest`. Not yet tested in `eu-west-1`. Omit encryption headers — server-side encryption is applied automatically. |
...
| Server-side copy | `CopyObject` supported. | Confirmed working in `us-east-1`. Not yet confirmed in `eu-west-1` — download and re-upload as a fallback there. |
...
| Object Lock modes | Compliance and Governance. | Compliance and Governance. Legal Hold confirmed working in `us-east-1`; not yet confirmed in `eu-west-1`. |
