...
2. On the **Buckets** tab, click **Index** on the bucket you want, then confirm with **Start indexing**.
3. Wait for the first indexing pass to complete. Each bucket shows a status of **Not indexed**, **Indexing**, **Failed**, or **Ready**, and a **Ready** bucket also shows the number of files indexed, the index size, and when it last synced.

See [Bucket Intelligence in the console](/app/bucket-intelligence) for the full walkthrough of the dashboard, including asking questions and managing keys there.
...
| Feature | Status |
|---------|--------|
| MCP endpoint / MCP server for Claude, Cursor, and other clients | Not built, and no longer listed in the dashboard. A hosted connection layer for AI clients is what the [AI Agent Toolkit](/app/ai-agent-toolkit) is intended to provide, and it is also not released yet. |
