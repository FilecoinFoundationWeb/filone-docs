...
When you delete an object or bucket, it is immediately removed from your Fil One account and is no longer accessible via the API or dashboard. Because Filecoin stores data in sealed sectors, the underlying storage may persist for a bounded period until the sector naturally expires. That residual data stays encrypted and is unreachable through Fil One from the moment of deletion, though note that the key is held by the regional storage operator, so this is loss of access rather than cryptographic destruction.

Closing an account revokes access rather than erasing data. Erasure is a separate manual request, and what it can achieve at the storage layer depends on your region and is currently being confirmed. For the full picture, including versioned buckets, Object Lock, account closure, and residual data, see [Data Deletion](/security/data-deletion).
