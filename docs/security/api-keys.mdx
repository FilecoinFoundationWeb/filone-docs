...
4. Choose the key's **region** (see [Region scope](#region-scope)).
5. Set the key's permissions and bucket scope (see below).
...
| **List** | `ListBuckets`, `ListObjectsV2` |
| **Delete** | `DeleteObject` |
...
For buckets with [versioning](/storage/versioning) or [Object Lock](/storage/object-lock) enabled, you can enable additional granular permissions on top of the basic ones:
...
| `PutObjectRetention` | Write | Apply or extend retention policies |
...
| Region | The region the key is scoped to |
