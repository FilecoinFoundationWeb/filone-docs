...
**Do not send encryption headers.** Fil One applies its own server-side encryption, and you cannot request a specific mode. Uploads specifying `ServerSideEncryption: aws:kms` (SSE-KMS) are **rejected** in `us-east-1` with `InvalidRequest`; this has not been tested in `eu-west-1`. Omit the header entirely — encryption is applied either way.
